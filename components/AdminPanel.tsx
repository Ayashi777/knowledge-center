...
export const AdminPanel: React.FC<{ 
    ...
    const handleDeleteUser = async (uid: string) => {
        ...
        try {
            ...
            // Note: This only deletes from Firestore. Auth user deletion requires Admin SDK or Cloud Function.
                            ...
                            <div className="animate-fade-in">
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="border-b border-gray-200 dark:border-gray-700 text-xs text-gray-400 uppercase tracking-widest">
                                                <th className="py-4 px-2">Користувач</th>
                                                <th className="py-4 px-2">Компанія</th>
                                                <th className="py-4 px-2">Контакти</th>
                                                <th className="py-4 px-2">Роль</th>
                                                <th className="py-4 px-2 text-right">Дії</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                            {users.map(user => (
                                                <tr key={user.uid} className="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                                                    <td className="py-4 px-2">
                                                        <p className="font-bold text-gray-900 dark:text-white text-sm">{user.name || 'Без імені'}</p>
                                                        <p className="text-xs text-gray-500">{user.email}</p>
                                                    </td>
                                                    <td className="py-4 px-2">
                                                        <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">{user.company || '—'}</p>
                                                    </td>
                                                    <td className="py-4 px-2">
                                                        {user.phone ? (
                                                            <a href={`tel:${user.phone}`} className="text-xs font-bold text-blue-600 hover:underline">{user.phone}</a>
                                                    ...
                                                    <td className="py-4 px-2">
                                                        <select 
                                                            ...
                                                            className="bg-gray-100 dark:bg-gray-800 border-none rounded-lg text-xs font-black uppercase px-2 py-1 cursor-pointer focus:ring-2 focus:ring-blue-500 outline-none"
                                                        >
                                                            {roles.map(r => <option key={r} value={r}>{t(`roles.${r}`)}</option>)}
                                                        </select>
                                                    </td>
                                                    <td className="py-4 px-2 text-right">
                                                        <button onClick={() => handleDeleteUser(user.uid)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Видалити користувача">
