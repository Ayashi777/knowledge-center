...
export const DocumentThumbnail: React.FC<{ docTitle: string }> = ({ docTitle }) => (
...
export const DocumentListItem = forwardRef<HTMLDivElement, { doc: Document, onClick: () => void, onEdit: () => void, onDelete: () => void, showAdminControls: boolean }>(({ doc, onClick, onEdit, onDelete, showAdminControls }, ref) => {
    ...
    <div ref={ref} onClick={onClick} onKeyPress={(e) => e.key === 'Enter' && onClick()} className="flex items-start sm:items-center justify-between p-4 bg-white/80 dark:bg-gray-800/50 border-b border-gray-200/50 dark:border-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200 rounded-md group flex-col sm:flex-row gap-4" role="button" tabIndex={0}>
      ...
      {showAdminControls ? (
        <div className="flex items-center gap-2 pl-0 sm:pl-4 self-end sm:self-center">
          <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-2 text-gray-400 dark:text-gray-500 hover:text-blue-500 dark:hover:text-blue-400 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fillRule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clipRule="evenodd" /></svg>
          </button>
          <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-2 text-gray-400 dark:text-gray-500 hover:text-red-500 transition-colors">
...
export const DocumentGridItem: React.FC<{ doc: Document; onClick: () => void; onRequireLogin: () => void; isGuest: boolean }> = ({ doc, onClick, onRequireLogin, isGuest }) => {
  const { t } = useI18n();
  const docTitle = doc.titleKey ? t(doc.titleKey) : doc.title || '';
