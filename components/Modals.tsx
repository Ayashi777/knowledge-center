                ...
                <div className="flex-1 p-8 md:p-12 bg-white dark:bg-gray-800 flex flex-col justify-center min-h-[500px]">
                            ...
                            <form onSubmit={handleRequestSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div className="sm:col-span-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-2 bg-gray-50/50 dark:bg-black/20 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
                                        <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-tighter transition-all ${hasMinLen ? 'text-green-500 scale-105' : 'text-gray-400'}`}><Icon name={hasMinLen ? 'check-circle' : 'hr'} className="w-3 h-3" />8+ символів</div>
                                        <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-tighter transition-all ${hasNumber ? 'text-green-500 scale-105' : 'text-gray-400'}`}><Icon name={hasNumber ? 'check-circle' : 'hr'} className="w-3 h-3" />Цифра</div>
                                        <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-tighter transition-all ${hasUpper ? 'text-green-500 scale-105' : 'text-gray-400'}`}><Icon name={hasUpper ? 'check-circle' : 'hr'} className="w-3 h-3" />Велика літера</div>
                                        <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-tighter transition-all ${hasLower ? 'text-green-500 scale-105' : 'text-gray-400'}`}><Icon name={hasLower ? 'check-circle' : 'hr'} className="w-3 h-3" />Мала літера</div>
                                    </div>
                                </div>
                    ...
                    {view === 'success' && (
                        <div className="animate-fade-in text-center py-10">
                            <div className="w-24 h-24 bg-green-50 dark:bg-green-900/30 rounded-full flex items-center justify-center mx-auto mb-8 text-green-600 shadow-inner"><Icon name="check-circle" className="w-12 h-12" /></div>
...
export const DocumentEditorModal: React.FC<{ doc: Partial<Document> | null, onSave: (doc: Partial<Document>) => void, onClose: () => void, availableCategories: Category[] }> = ({ doc, onSave, onClose, availableCategories }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSave({ id: doc?.id, title: title, categoryKey: category, tags: tags.split(',').map(t => t.trim()).filter(Boolean) }); };
    return (
...
export const CategoryEditorModal: React.FC<{ category: Category, onSave: (cat: Category) => void, onClose: () => void }> = ({ category, onSave, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl w-full max-lg p-8 border border-gray-200 dark:border-gray-700" onClick={e => e.stopPropagation()}>
